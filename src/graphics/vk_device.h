...
namespace lincore
{
	...
	public:
		...
		T* GetResource(ResourceHandle handle)
		{
			...
			// {
			// 	return resource_manager_.GetPipeline({handle});
			// }
			...
			// {
			// 	return resource_manager_.GetDescriptorSetLayout({handle});
			// }
			// else if constexpr (std::is_same_v<T, DescriptorSet>)
			// {
			// 	return resource_manager_.GetDescriptorSet({handle});
			// }
			// else if constexpr (std::is_same_v<T, RenderPass>)
			// {
			// 	return resource_manager_.GetRenderPass({handle});
			// }
			// else if constexpr (std::is_same_v<T, Framebuffer>)
			// {
			// 	return resource_manager_.GetFramebuffer({handle});
			// }
			// else if constexpr (std::is_same_v<T, ShaderState>)
			// {
			// 	return resource_manager_.GetShaderState({handle});
			// }
			else
			{
		...
		auto CreateResource(const CreationType& creation)
		{
			...
			// else if constexpr (std::is_same_v<CreationType, PipelineCreation>)
			// {
			// 	return resource_manager_.CreatePipeline(creation);
			// }
			// else if constexpr (std::is_same_v<CreationType, SamplerCreation>)
			// {
			// 	return resource_manager_.CreateSampler(creation);
			// }
			// else if constexpr (std::is_same_v<CreationType, DescriptorSetLayoutCreation>)
			// {
			// 	return resource_manager_.CreateDescriptorSetLayout(creation);
			// }
			// else if constexpr (std::is_same_v<CreationType, DescriptorSetCreation>)
			// {
			// 	return resource_manager_.CreateDescriptorSet(creation);
			// }
			// else if constexpr (std::is_same_v<CreationType, RenderPassCreation>)
			// {
			// 	return resource_manager_.CreateRenderPass(creation);
			// }
			// else if constexpr (std::is_same_v<CreationType, FramebufferCreation>)
			// {
			// 	return resource_manager_.CreateFramebuffer(creation);
			// }
			// else if constexpr (std::is_same_v<CreationType, ShaderStateCreation>)
			// {
			// 	return resource_manager_.CreateShaderState(creation);
			// }
			// else if constexpr (std::is_same_v<CreationType, TextureViewCreation>)
			// {
			// 	return resource_manager_.CreateTextureView(creation);
			// }
			else
			{
		...
		void DestroyResource(HandleType handle)
		{
			...
			else if constexpr (std::is_same_v<HandleType, PipelineHandle>)
			{
				resource_manager_.DestroyPipeline(handle);
			}
			else if constexpr (std::is_same_v<HandleType, SamplerHandle>)
			{
				resource_manager_.DestroySampler(handle);
			}
			else if constexpr (std::is_same_v<HandleType, DescriptorSetLayoutHandle>)
			{
				resource_manager_.DestroyDescriptorSetLayout(handle);
			}
			else if constexpr (std::is_same_v<HandleType, DescriptorSetHandle>)
			{
				resource_manager_.DestroyDescriptorSet(handle);
			}
			else if constexpr (std::is_same_v<HandleType, RenderPassHandle>)
			{
				resource_manager_.DestroyRenderPass(handle);
			}
			else if constexpr (std::is_same_v<HandleType, FramebufferHandle>)
			{
				resource_manager_.DestroyFramebuffer(handle);
			}
			else if constexpr (std::is_same_v<HandleType, ShaderStateHandle>)
			{
				resource_manager_.DestroyShaderState(handle);
			}
			else
			{
