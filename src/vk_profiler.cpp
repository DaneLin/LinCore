...
vkutils::VulkanScopeTimer::~VulkanScopeTimer()
{
...
vkutils::VulkanPipelineStatRecorder::~VulkanPipelineStatRecorder()
{
    VkQueryPool pool = profiler_->GetStatPool();
...
void vkutils::VulkanProfiler::Init(VkDevice device, float time_stamp_period, int per_frame_pool_sizes)
{
    ...
    device_ = device;
    current_frame_ = 0;
    int pool_sizes = per_frame_pool_sizes;

    VkQueryPoolCreateInfo query_pool_info = {};
    query_pool_info.sType = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO;
    query_pool_info.queryType = VK_QUERY_TYPE_TIMESTAMP;
    query_pool_info.queryCount = pool_sizes;
...
void vkutils::VulkanProfiler::GrabQueries(VkCommandBuffer commands)
{
...
void vkutils::VulkanProfiler::CleanUp()
{
    for (int i =0; i < kQUERY_FRAME_OVERLAP; ++i)
    {
...
VkQueryPool vkutils::VulkanProfiler::GetTimerPool()
{
    return query_frames_[current_frame_].timer_pool;
}

VkQueryPool vkutils::VulkanProfiler::GetStatPool()
{
...
uint32_t vkutils::VulkanProfiler::GetTimeStampId()
{
...
uint32_t vkutils::VulkanProfiler::GetStatId()
{
