...
struct CommandRecordingTask : enki::ITaskSet {
	...
	virtual void ExecuteRange(enki::TaskSetPartition range, uint32_t threadnum) override {
		...
		inheritance_info.color_formats = &engine->gpu_device_.draw_image_.format;
		inheritance_info.depth_format = engine->gpu_device_.depth_image_.format;
...
#if LC_DRAW_INDIRECT
		thread_cmd->BindIndexBuffer(engine->gpu_device_.resource_manager_.GetBuffer(engine->global_mesh_buffer_.index_buffer_handle).buffer, 0, VK_INDEX_TYPE_UINT32);
#endif
...
void VulkanEngine::Init()
{
	...
	{
		auto scene = lc::LoadGltf(&gpu_device_, path);
	...
	/*gpu_device_.UploadBuffer(global_mesh_buffer_.vertex_buffer_handle,
		global_mesh_buffer_.vertex_data.data(),
		global_mesh_buffer_.vertex_data.size() * sizeof(Vertex),
	...
	gpu_device_.UploadBuffer(global_mesh_buffer_.index_buffer_handle,
		global_mesh_buffer_.index_data.data(),
		global_mesh_buffer_.index_data.size() * sizeof(uint32_t),
		VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
	gpu_device_.UploadBuffer(global_mesh_buffer_.indirect_command_buffer_handle,
		global_mesh_buffer_.indirect_commands.data(),
		global_mesh_buffer_.indirect_commands.size() * sizeof(VkDrawIndexedIndirectCommand),
		VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);*/

		//gpu_device_.SetDebugName(VkObjectType::VK_OBJECT_TYPE_BUFFER, global_mesh_buffer_.vertex_buffer_handle.index, "Global Vertex Buffer");
		// gpu_device_.SetDebugName(VkObjectType::VK_OBJECT_TYPE_BUFFER, global_mesh_buffer_.index_buffer_handle.index, "Global Index Buffer");
		// gpu_device_.SetDebugName(VkObjectType::VK_OBJECT_TYPE_BUFFER, global_mesh_buffer_.indirect_command_buffer_handle.index, "Global Indirect Buffer");
...
void VulkanEngine::CleanUp()
{
	if (is_initialized_)
	{
		...
		{
			vkDestroyFence(gpu_device_.device_, frames_[i].render_fence, nullptr);
			vkDestroySemaphore(gpu_device_.device_, frames_[i].render_semaphore, nullptr);
			vkDestroySemaphore(gpu_device_.device_, frames_[i].swapchain_semaphore, nullptr);

			frames_[i].deletion_queue.Flush();
		}*/

		metal_rough_material_.ClearResources(gpu_device_.device_);
		main_deletion_queue_.Flush();

		gpu_device_.Shutdown();
		// vkDestroySurfaceKHR(instance_, surface_, nullptr);
		// vkDestroyDevice(gpu_device_.device_, nullptr);
...
void VulkanEngine::Draw()
{
	...
	gpu_device_.command_buffer_manager_.ResetPools(gpu_device_.frame_number_ % kFRAME_OVERLAP);
	CommandBuffer* cmd = gpu_device_.command_buffer_manager_.GetCommandBuffer(gpu_device_.frame_number_ % kFRAME_OVERLAP, 0, true);

	gpu_device_.draw_extent_.width = static_cast<uint32_t>(std::min(gpu_device_.swapchain_extent_.width, gpu_device_.draw_image_.extent.width) * gpu_device_.render_scale_);
	gpu_device_.draw_extent_.height = static_cast<uint32_t>(std::min(gpu_device_.swapchain_extent_.height, gpu_device_.draw_image_.extent.height) * gpu_device_.render_scale_);

	{
		gpu_device_.profiler_.GrabQueries(cmd->GetCommandBuffer());

		cmd->TransitionImage(gpu_device_.draw_image_.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);

		DrawBackground(cmd);

		cmd->TransitionImage(gpu_device_.draw_image_.image, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
		cmd->TransitionImage(gpu_device_.depth_image_.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL);

		DrawGeometry(cmd);

		cmd->TransitionImage(gpu_device_.draw_image_.image, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
		...
		cmd->CopyImageToImage(gpu_device_.draw_image_.image, gpu_device_.swapchain_images_[swapchain_image_index], gpu_device_.draw_extent_, gpu_device_.swapchain_extent_);
	...
	gpu_device_.command_buffer_manager_.ImmediateSubmit([&](CommandBuffer* cmd)
		{
			...
			vkCmdCopyBuffer(cmd->command_buffer_, staging.buffer, gpu_device_.resource_manager_.GetBuffer(new_surface.vertex_buffer_handle).buffer, 1, &vertex_copy);
			...
			vkCmdCopyBuffer(cmd->command_buffer_, staging.buffer, gpu_device_.resource_manager_.GetBuffer(new_surface.index_buffer_handle).buffer, 1, &index_copy); }, gpu_device_.graphics_queue_);
...
void VulkanEngine::InitBackgounrdPipelines()
{
	...
	gradient.descriptor_binder.BindImage("image", { .sampler = VK_NULL_HANDLE, .imageView = gpu_device_.draw_image_.view, .imageLayout = VK_IMAGE_LAYOUT_GENERAL });
	...
	sky.descriptor_binder.BindImage("image", { .sampler = VK_NULL_HANDLE, .imageView = gpu_device_.draw_image_.view, .imageLayout = VK_IMAGE_LAYOUT_GENERAL });
...
void VulkanEngine::InitDefaultData()
{
	...
	material_resources.color_image = gpu_device_.resource_manager_.GetTexture(gpu_device_.default_resources_.images.white_image);
	material_resources.color_sampler = gpu_device_.default_resources_.samplers.linear;
	material_resources.metal_rough_image = gpu_device_.resource_manager_.GetTexture(gpu_device_.default_resources_.images.white_image);
	...
	dafault_data_ = metal_rough_material_.WriteMaterial(gpu_device_.device_, MeshPassType::kMainColor, material_resources, gpu_device_.descriptor_allocator_);
}
...
{
	vkutils::VulkanScopeTimer timer(cmd->GetCommandBuffer(), &gpu_device_.profiler_, "Background");
	vkutils::VulkanPipelineStatRecorder timers(cmd->GetCommandBuffer(), &gpu_device_.profiler_, "Background Primitives");
...
{
	vkutils::VulkanScopeTimer timer(cmd->command_buffer_, &gpu_device_.profiler_, "ImGui");
	vkutils::VulkanPipelineStatRecorder timers(cmd->command_buffer_, &gpu_device_.profiler_, "ImGui Primitives");
...
{
	vkutils::VulkanScopeTimer timer(cmd->command_buffer_, &gpu_device_.profiler_, "Geometry");
	vkutils::VulkanPipelineStatRecorder timers(cmd->command_buffer_, &gpu_device_.profiler_, "Geometry Primitives");
	// reset counters
	...
	std::sort(opaqueDraws.begin(), opaqueDraws.end(), [&](const auto& iA, const auto& iB)
		{
			...
			if (A.material == B.material) {
				return gpu_device_.resource_manager_.GetBuffer(A.index_buffer_handle).buffer < gpu_device_.resource_manager_.GetBuffer(B.index_buffer_handle).buffer;
			}
			else {
			...
			*scene_uniform_data = gpu_device_.scene_data_;
...
#if LC_DRAW_INDIRECT
				writer.WriteBuffer(1, gpu_device_.resource_manager_.GetBuffer(global_mesh_buffer_.vertex_buffer_handle).buffer, sizeof(Vertex) * global_mesh_buffer_.vertex_data.size(), 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
#endif // LC_DRAW_INDIRECT
				writer.UpdateSet(gpu_device_.device_, global_descriptor);
			}

			// begin a render pass connected to our draw image
			VkRenderingAttachmentInfo color_attachment = vkinit::AttachmentInfo(gpu_device_.draw_image_.view, nullptr, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
			VkRenderingAttachmentInfo depth_attachment = vkinit::DepthAttachmentInfo(gpu_device_.depth_image_.view, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL);
			...
			for (uint32_t thread_idx = 0; thread_idx < num_threads; ++thread_idx) {
				secondary_cmds[thread_idx] = gpu_device_.command_buffer_manager_.GetSecondaryCommandBuffer(
					gpu_device_.frame_number_ % kFRAME_OVERLAP,
					thread_idx);
			}
			...
			for (uint32_t thread = 0; thread < num_threads; ++thread)
			{
				...
				recordTask.frame_index = gpu_device_.frame_number_ % kFRAME_OVERLAP;
...
void VulkanEngine::DrawObject(CommandBuffer* cmd, const RenderObject& r, RenderInfo& render_info)
{
	...
	cmd->DrawIndexedIndirect(gpu_device_.resource_manager_.GetBuffer(global_mesh_buffer_.indirect_command_buffer_handle).buffer,
...
void GLTFMetallic_Roughness::BuildPipelines(VulkanEngine* engine)
{
	...
	// render format
	pipelineBuilder.SetColorAttachmentFormat(engine->gpu_device_.draw_image_.format);
	pipelineBuilder.SetDepthFormat(engine->gpu_device_.depth_image_.format);
	...
	engine->gpu_device_.command_buffer_manager_.ImmediateSubmit([&](CommandBuffer* cmd)
		{
			...
			vkCmdCopyBuffer(cmd->command_buffer_, staging_buffer.buffer, engine->gpu_device_.resource_manager_.GetBuffer(vertex_buffer_handle).buffer, 1, &vertex_copy);
			...
			vkCmdCopyBuffer(cmd->command_buffer_, staging_buffer.buffer, engine->gpu_device_.resource_manager_.GetBuffer(index_buffer_handle).buffer, 1, &index_copy);
			...
			vkCmdCopyBuffer(cmd->command_buffer_, staging_buffer.buffer, engine->gpu_device_.resource_manager_.GetBuffer(indirect_command_buffer_handle).buffer, 1, &command_copy);
		},
